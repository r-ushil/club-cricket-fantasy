...
export default function HomeTabs({ userTeamInfo, teams }: HomeTabsProps) {
  ...
  return (
    <Tabs value={tab} onValueChange={setTab} className="lg:w-2/3 w-full mx-auto px-4 lg:px-16 pb-10">
      <TabsList className="relative flex justify-center bg-white/5 backdrop-blur-md border border-zinc-700 rounded-xl shadow-sm overflow-hidden w-fit mx-auto mb-6">
        <TabsTrigger
          value="team"
          className="group relative px-6 py-2 text-sm md:text-base font-medium text-zinc-300 transition-all duration-300 ease-in-out hover:text-white data-[state=active]:text-white"
        >
          My Team
