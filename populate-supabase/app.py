...
def merge_dicts(players, batters, bowlers, fielders):
    new_dict = {}
    all_keys = set(players.keys()) | set(batters.keys()) | set(bowlers.keys()) | set(fielders.keys())

    for key in all_keys:
        new_dict[key] = players.get(key, 0) + batters.get(key, 0) + bowlers.get(key, 0) + fielders.get(key, 0)
